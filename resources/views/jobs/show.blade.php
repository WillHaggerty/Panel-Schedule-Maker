    ...
    <div class="col-md-8">
      <div class="card">
        <div class="card-header">
          ...
          <div class="float-right"><a class=" btn btn-info" href="/jobs">Job List</a></div>
        </div>
        ...
        <div class="card-body">
          <ul class="list-group list-group-flush">
            ...
            <li class="list-group-item"><a href="/jobs/{{ $job->id }}/panels/{{ $panel->id }}">{{ $panel->name }}</a>{{ empty($panel->info) ? '' : ': '.$panel->info }}</li>
            @endforeach
          </ul>
        </div>
        @else
        ...
        <div class="card-footer">
          <div class="float-left">
            <a class="btn btn-success" href="/jobs/{{ $job->id }}/panels/create" role="button">Add panel</a>
          </div>
          <div class="float-right">
            <form method="POST" action="/jobs/{{ $job->id }}">
              @method('DELETE')
              @csrf
              <a class="btn btn-primary" href="/jobs/{{ $job->id }}/edit" role="button">Edit job</a>
